  ...
  useEffect(() => {
    async function fetchFavorites() {
      try {
        ...
        const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/favorites`, {
        ...
        const moviePromises = favs.map(async (fav) => {
          try {
            const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/tmdb/movie/${fav.movieId}`);
          ...
          {movies.map((movie) => {
            ...
            return (
              <div key={movie.id} className="bg-blue-800 p-4 rounded shadow hover:shadow-lg transition">
                ...
                <p className="text-gray-200 mb-1"><strong>Release:</strong> {movie.release_date}</p>
                <p className="text-gray-200 mb-1"><strong>Rating:</strong> {movie.vote_average}</p>
                <p className="text-gray-200 mb-1 line-clamp-3">{movie.overview}</p>
                <Link href={`/movies/tmdb/${movie.id}`} className="mt-3 inline-block bg-yellow-400 text-black px-3 py-1 rounded hover:bg-yellow-300 transition-colors">
                  View Details
                </Link>
              </div>
            );
